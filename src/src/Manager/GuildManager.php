...
class GuildManager
{
    ...
    public function getGuildFilterList(): Response
    {
        $query = '
            SELECT
              g.id,
              COALESCE(g.name, gm.name) AS name
            FROM guild g
            LEFT JOIN guild_meta gm
              ON g.id = gm.id
            INNER JOIN player p
              ON g.id = p.guild_id
            GROUP BY g.id, COALESCE(g.name, gm.name)
            ORDER BY COALESCE(g.name, gm.name), g.id;
        ';
    ...
    {
        $query = '
            SELECT COALESCE(g.name, gm.name) AS name
            FROM guild g
            LEFT JOIN guild_meta gm
              ON g.id = gm.id
            WHERE g.id = :guild_id
    ...
    public function getGuildsDirectory(): Response
    {
        $query = '
            ...
            LEFT JOIN (
              ...
              ON g.id = gf.guild_id
            GROUP BY p.guild_id, COALESCE(g.name, gm.name), gm.logo, alpha
            ORDER BY members DESC, alpha DESC, COALESCE(g.name, gm.name);
        ';
